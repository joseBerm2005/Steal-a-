...
function NPCManager:RequestNPCRedirect(npcID, player)
	...
	if totalTags >= #Pads:GetChildren() then
		print("yes")
		Pads = playerBase.Floor2.Interactables.Pads
	end
	...
	if RandomPad:GetAttribute("NPC") then NPCManager:RequestNPCRedirect(npcID,player) return end
	
	local basePosition = RandomPad.MainPart.Position
	
	RandomPad:SetAttribute("NPC", NPC_NAME)
	RandomPad:SetAttribute("NPCID", npcID)
					...
					if GetPlayerBase then
						...
						for _, spots in pairs(GetPlayerBase:GetDescendants()) do
							if spots.Parent.Name == "Pads" then
								if spots:GetAttribute("NPCID") == tonumber(npcID) then
									print("has attribute")
						...
						spotkey:SetAttribute("NPC", state.NpcName)
