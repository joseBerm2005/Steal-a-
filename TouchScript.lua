...
function MoneyManager.collectPendingMoney(player, collectpad)
	...
	for npcid, info in pairs(PendingMoneyTbl) do
		...
		PendingMoneyTbl[npcid].spot.Collect.BillboardGui.TextLabel.Text = "Collect: <br/> <font color='#53ff1e'> 0 </font>"
	end
...
local function VerifyCooldownAndBase()
...
function TouchHandlers.handleCollectPadTouch(hit, collectPad)
	...
	MoneyManager.collectPendingMoney(player, collectPad.Parent)
...
function ConnectionManager.setupPadConnections(item)
	print(item)
	if item.Name == "MainPart" then
		item.Touched:Connect(function(hit)
			TouchHandlers.handleMainPadTouch(hit, item)
		end)

	elseif item.Name == "Collect" then
		print(item)
		item.Touched:Connect(function(hit)
