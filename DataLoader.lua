...
--// State
local PlayerData = ProfileStore.New("PlayerDataV4", DataTemplate)
...
local function loadAllNPCs(player: Player)
	
	repeat task.wait() until player.Character
	
	if not CachedReplicas[player] then repeat task.wait() until CachedReplicas[player] end
	
	local NPCManager = require(game.ServerScriptService.Modules.Core.MoveServer.NPCManager)
	local Bases = require(game.ServerScriptService.Modules.Core.Bases)
	local NetworkManager = require(game.ServerScriptService.Modules.Core.MoveServer.NetworkManager)
	
	local plrData = CachedReplicas[player].Data
	
	local GetPlayerBase
	
	if Bases.Get_Player_Plot(player) then
		repeat task.wait() GetPlayerBase = Bases.Get_Player_Plot(player) until GetPlayerBase ~= nil
		
		print("base loaded")
	end
	
	
	if plrData.NPCS ~= {} then
		print("NPCSSSS YUHHHH")
		for NpcName, npcData in pairs(plrData.NPCS) do
			local npcID = NPCManager.NextNPCID
			
			print("yes going")
			
			for i,Spot in pairs(GetPlayerBase:GetDescendants()) do
				
				if Spot.Parent.Name ~= "Pads" then continue end
				
				print("parenting YAYYYYYY")
				
				for i,v in pairs(npcData.spots) do
					print(v)
					if Spot.Name == tostring(v) then
						print("found that sucker frrrr")
						
						NPCManager.NPCsInPlots[NPCManager.NextNPCID] = {
							player = player,
							Base = GetPlayerBase,
							spot = Spot,
							npcName = NpcName
						...
						print(NPCManager.NPCsInPlots[NPCManager.NextNPCID])
						
						Spot:SetAttribute("NPCID", npcID)
						Spot:SetAttribute("NPC", NpcName)
						game.ReplicatedStorage.Events.NPCPlacement:FireAllClients(npcID, Spot, player.Character, NpcName)
					end
				end
			end
	...
	if NPCManager.NPCsInPlots ~= {} then
		for id, info in pairs(NPCManager.NPCsInPlots) do
			...
			if not info.spot then continue end
			if not info.player then continue end
			if not info.npcName then continue end
			
			if not info.spot:GetAttribute("NPCID") then
				print("duhk")
				info.spot:SetAttribute("NPCID", id)
			end

			if not info.spot:GetAttribute("NPC") then
				info.spot:SetAttribute("NPC", info.npcName)
			end
			game.ReplicatedStorage.Events.NPCPlacement:FireAllClients(id, info.spot, info.player, info.npcName)
		end
	end
...
Players.PlayerRemoving:Connect(function(player)
	...
	for id, info in pairs(NPCManager.NPCsInPlots) do
		...
		if profile.Data.NPCS[info.npcName] then
			print("yoooo data frrr")

			profile.Data.NPCS[info.npcName].count += 1

			table.insert(profile.Data.NPCS[info.npcName].spots, info.spot.Name)

		else

			profile.Data.NPCS[info.npcName] = {
				spots = {info.spot.Name},
				count = 1,
			}
