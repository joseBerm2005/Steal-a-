...
function NPCManager:CreateIdleNPC(npcID, NpcName : string, spot, npc : Model)
	if spot == nil then
		repeat task.wait() spot = findSpotwithCorrectID(npcID) until spot ~= nil
	end
	
	
	if npc == nil then
		local npcClone = ReplicatedStorage.Assets.NPCs[NpcName]:Clone()
		
		npcClone.Parent = game.Workspace.NPCS
		
		npcClone:SetAttribute("NPCID", npcID)
	...
	local npcClone = rig:Clone()
	npcClone.Name = NpcName.."NPC"

	npcClone.Parent = game.Workspace.NPCS
	
	npcClone:SetAttribute("NPC_ID", tonumber(npcID))

	local humanoidDescription = Players:GetHumanoidDescriptionFromUserId(player.UserId)
	local npcHumanoid = npcClone:FindFirstChildOfClass("Humanoid")
	...
	if npc and npcClone:FindFirstChild("RightHand") and npc.PrimaryPart then
		...
		motor6D.Part0 = npcClone.RightHand
		motor6D.Part1 = npc.PrimaryPart
		motor6D.Parent = npcClone.RightHand

		motor6D.C0 = CFrame.new(0, -1, 0) * CFrame.Angles(math.rad(-90), 0, 0)
		motor6D.C1 = CFrame.new(0, 0, 0)
	end
	
	npc.PrimaryPart.Massless = true

	npcClone:PivotTo(spot.MainPart.CFrame)

	-- Anchor the NPC to prevent physics from affecting it
	local humanoidRootPart = npcClone:FindFirstChild("HumanoidRootPart")
	if humanoidRootPart then
		humanoidRootPart.Anchored = true
	end

	if npcClone:FindFirstChild("Humanoid") then

		local animator : Animator = npcClone.Humanoid.Animator

		local animation = animator:LoadAnimation(ReplicatedStorage.Animations.EatingAnim)
...
function NPCManager.Create_BBG(NPC : typeof(ReplicatedStorage.Assets.NPCs.Jandel))
	...
	NewBBG.Parent = NPC
	NewBBG.Adornee = NPC:FindFirstChildWhichIsA("MeshPart") or NPC:FindFirstChildWhichIsA("BasePart")
end
...
function NPCManager:CreateClientNPC(npcID, startTime, duration, currentPosition, NpcName: string)
	...
	Promixity.new(newNPC:FindFirstChildWhichIsA("BasePart"), "Buy")
			...
			if data.IsRedirected and data.Waypoints and #data.Waypoints > 0 then
				...
				if t >= 1 then
					...
					if #data.Waypoints > 0 then
						...
						NPCManager:CreateIdleNPC(npcID,npc.Name, nil, npc)
					end
				end
